...
import 'package:peminjaman/model/pengajuan_form.dart';
...
class PengajuanFormController extends GetxController {
...
List<PengajuanForm> listPengajuanForm = <PengajuanForm>[].obs;
late PeminjamanLab dataDetailPeminjamanLab ;
  ...
  detailPeminjamanLab(int id) {
    //  dataDetailPeminjamanLab = listPengajuanForm.firstWhere((element) => element.id == id,);
  ...
  listDataPengajuan() async {
    ...
    try {
      var response = await http.get(Uri.parse(ApiServices.listPengajuanForm),
          headers: header);
      if (response.statusCode == 200) {
        Map<String, dynamic> data = jsonDecode(response.body);
        List result = data['data'];
         listPengajuanForm = result.map((items) => PengajuanForm.fromJson(items)).toList();
        isDataReadingCompleted.value = true;
        isLoading.value = false;
      } else {
        isLoading.value = false;
  ...
  saveData() async {
    ...
    try {
      var response = await http.post(Uri.parse(ApiServices.savePengajuanForm),headers: header, body: body);
